/*
*********************************************************************************************************
*                                    uC/OS-III on the Atmel ATxmega128A1
*********************************************************************************************************
*
...
* Programmer:       Atmel JTAGICE mkII (via JTAG)
* Debugger:         Atmel JTAGICE mkII (via JTAG)
...
*********************************************************************************************************
*
* 1.    Many features have been disabled in os_cfg.h in order to conserve SRAM space.
* 2.    os_stat.c and and os_int.c have been modified to prevent compiler errors and warnings respectively.
* 3.    CPU clock frequency can be changed in Atmel Studio 6 via Project Properties (in the AVR/GNU C Compiler and AVR/GNU Assembler "Symbols" sections). Default is 32 MHz.
*       Valid frequencies = [32000000,2000000]
* 4.    Choose USART to print to in includes.h
*
*********************************************************************************************************
...
* SW3:  Prints the tick rate (OS_CFG_TICK_RATE_HZ) and the USART RX ring buffer size (in bytes) (USARTXX_RX_BUFFER_SIZE_BYTES) to USARTXX.
...
// uC/OS-III semaphores
OS_SEM pushbutton_event;

// uC/OS-III error variable
OS_ERR err;
...
void AppTaskStart_CreateTasks(void);
...
int main(void)
{
    ...
    // Setup the default stack size
    OSTaskStkSize     = OS_CFG_IDLE_TASK_STK_SIZE;
...
// AppTaskStart: PRIORITY=APP_TASK_START_PRIO
static void AppTaskStart(void *p_arg)
{
    ...
    while(1)
    {
        ...
        switch(buttonState)
        {
            ...
            case 0x02:  printf("Push-Button 1 press detected. Clock Speed=%ld Hz\n",F_CPU);
                        break;
            case 0x04:  printf("Push-Button 2 press detected. OS_VERSION=%d, CPU_CFG_CRITICAL_METHOD=%d\n",OS_VERSION, CPU_CFG_CRITICAL_METHOD);
                        break;
            #if PRINT_TO_USART == C0
            case 0x08:  
                        printf("Push-Button 3 press detected. OS_CFG_TICK_RATE_HZ=%d, USARTC0_RX_BUFFER_SIZE_BYTES=%d bytes\n",OS_CFG_TICK_RATE_HZ, USARTC0_RX_BUFFER_SIZE_BYTES);
                        break;
            #elif PRINT_TO_USART == D0
            case 0x08:  
                        printf("Push-Button 3 press detected. OS_CFG_TICK_RATE_HZ=%d, USARTD0_RX_BUFFER_SIZE_BYTES=%d bytes\n",OS_CFG_TICK_RATE_HZ, USARTD0_RX_BUFFER_SIZE_BYTES);
                        break;          
            #endif
            case 0x10:  printf("Push-Button 4 press detected.\n");
                        break;
            case 0x20:  printf("Push-Button 5 press detected.\n");
                        break;
            case 0x40:  printf("Push-Button 6 press detected.\n");
                        break;
            case 0x80:  showCPUUsage=!showCPUUsage;
                        if(showCPUUsage)
                        {
                            printf("Push-Button 7 press detected. CPU Usage Display ON\n");
                            OSTaskResume(&AppTask4TCB,&err);
                        }
                        else
                        {
                            printf("Push-Button 7 press detected. CPU Usage Display OFF\n");
                            OSTaskSuspend(&AppTask4TCB,&err);
                        }
                        break;
        }
    }       
...
*             handlers written in assembly located in avr_isr.s. It is those handlers which perform the
...
*          3) Add the assembly ISR handler to avr_isr.s - use an existing ISR as a template.
...
*                   TestISR_1.
*               -   Create the following references near the beginning of the file (avr_isr.s):
*                           .global  TestISR
*                           .extern  TestISRHandler
*          4) Add the ISR handler in C below (xmega_ucos-iii.c).
...
void usartc0_rx_isr_handler(void)
{
    ...
    // Signal task to handle received packet
    OSTaskSemPost(&AppTask1TCB,OS_OPT_POST_NONE,&err);
...
void pushbutton_timer_isr_handler(void)
{
    ...
    if ((count>4)&&(edge))
    {
        ...
        edge=false; 
        // Post semaphore
        OSTaskSemPost(&AppTask5TCB,OS_OPT_POST_NONE,&err);
    }
