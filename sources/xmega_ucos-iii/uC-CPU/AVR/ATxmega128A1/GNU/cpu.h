...
#define  CPU_CFG_DATA_SIZE              CPU_WORD_SIZE_08        /* Defines CPU data    word size  (in octets).          */
...
typedef  CPU_INT08U             CPU_STK;                        /* Defines CPU stack word size (in octets).             */
...
typedef  CPU_INT08U                 CPU_SR;                     /* Defines   CPU status register size (see Note #3b).   */
...
#define  CPU_INT_DIS()         do { asm volatile ("cli"); } while (0)                 /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { asm volatile ("sei"); } while (0)                 /* Enable  interrupts.                          */
#endif
